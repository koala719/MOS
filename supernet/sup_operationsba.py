...
class OPS_containers():
    def __init__(self, n_class = 10, init_c=16, path = None):
        ...
        for i in range(layers):
            OPS_layers = { 0: lambda C_in, C_out, stride, affine: InvertedResidual(C_in, C_out, 3, stride, expand_ratio=3, is_use_ds=False),
                           1: lambda C_in, C_out, stride, affine: InvertedResidual(C_in, C_out, 3, stride, expand_ratio=3, is_use_ds=True),
                           2: lambda C_in, C_out, stride, affine: InvertedResidual(C_in, C_out, 5, stride, expand_ratio=3, is_use_ds=False),
                           3: lambda C_in, C_out, stride, affine: InvertedResidual(C_in, C_out, 5, stride, expand_ratio=3, is_use_ds=True),
                           4: lambda C_in, C_out, stride, affine: InvertedResidual(C_in, C_out, 7, stride, expand_ratio=3, is_use_ds=False),
                           5: lambda C_in, C_out, stride, affine: InvertedResidual(C_in, C_out, 7, stride, expand_ratio=3, is_use_ds=True),
                           6: lambda C_in, C_out, stride, affine: InvertedResidual(C_in, C_out, 3, stride, expand_ratio=6, is_use_ds=False),
                           7: lambda C_in, C_out, stride, affine: InvertedResidual(C_in, C_out, 3, stride, expand_ratio=6, is_use_ds=True),
                           8: lambda C_in, C_out, stride, affine: InvertedResidual(C_in, C_out, 5, stride, expand_ratio=6, is_use_ds=False),
                           9: lambda C_in, C_out, stride, affine: InvertedResidual(C_in, C_out, 5, stride, expand_ratio=6, is_use_ds=True),
                           10: lambda C_in, C_out, stride, affine: InvertedResidual(C_in, C_out, 7, stride, expand_ratio=6, is_use_ds=False),
                           11: lambda C_in, C_out, stride, affine: InvertedResidual(C_in, C_out, 7, stride, expand_ratio=6, is_use_ds=True),
            ...
            for j in range(ops):
                if i == 0:
                    OPS_layer[j] = OPS_layers[j](init_c, init_c * 2, 1, True)
                elif i in [1, 2, 3, 4]:
        ...
        self.stem = stem(3, 32, 1)
        self.separable_conv = separable_conv(32, init_c)


        self.conv_before_pooling = conv_before_pooling(256, self.last_channel)
