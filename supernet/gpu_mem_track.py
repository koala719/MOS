...
class MemTracker(object):
    ...
    def __init__(self, frame, detail=True, path='', verbose=False, device=0):
        ...
        self.gpu_profile_fn = path + df.strftime( '%Y-%m-%d %H:%M:%S %f' ) + "-gpu_mem_track.txt"
    ...
    def track(self):
        ...
        with open(self.gpu_profile_fn, 'a+') as f:
            ...
            if self.begin:
                ...
                f.write("GPU Memory Track | " + df.strftime( '%Y-%m-%d %H:%M:%S %f' ) + "|"
                        # f" Total Used Memory:{meminfo.used/1000**2:<7.1f}Mb\n\n")
                        "Total Used Memory:" + "{}".format(meminfo.used/1000**2) + "Mb")
                f.write('\n')
                self.begin = False
            ...
            if self.print_detail is True:
                ...
                for t, s, n, m in new_tensor_sizes - self.last_tensor_sizes:
                    ...
                    f.write('+ | {} * Size:{} | '.format(str(n),str(s)) + 'Memory: {} M | {}\n'.format(str(m*n), str(t)))
                    f.write('\n')
